  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center animate-in fade-in zoom-in duration-700">
        ...
        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Booking Initiated!</h1>
        <p className="text-slate-500 max-w-md mx-auto mt-4 text-lg">Your flight reservation has been created. We are redirecting you to the secure payment portal to finalize your ticket.</p>
        <button 
          onClick={() => window.location.href = '/dashboard/flights'}
          className="mt-10 px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-blue-600 transition-all"
        >
          Back to Flights
        </button>
      </div>
    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-6 py-4 bg-slate-50 rounded-2xl font-bold border-2 border-transparent focus:border-indigo-500/20 focus:bg-white outline-none transition-all"
                    value={contact.email}
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 text-white p-8 rounded-[40px] shadow-2xl shadow-slate-900/20 space-y-8 sticky top-10">
            <div className="space-y-2">
            ...
            <div className="space-y-4 border-t border-white/10 pt-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-black">₦{(parseFloat(price || '0') * 0.9).toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Taxes & Fees</span>
                <span className="text-sm font-black">₦{(parseFloat(price || '0') * 0.1).toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center pt-4 border-t border-white/10">
                <span className="text-xs font-black uppercase tracking-widest text-blue-400">Total Price</span>
                <span className="text-2xl font-black tracking-tighter">₦{(parseFloat(price || '0')).toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
               <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/10">
                  ...
                  <p className="text-[10px] font-bold text-slate-300 leading-relaxed uppercase tracking-tight">Secure Payment Guaranteed via Travsify Settle</p>
               </div>
            </div>
            ...
            <button 
              ...
              disabled={loading}
              className="w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-blue-600/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3"
            >
